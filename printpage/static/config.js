...
(() => {
  ...
  var getConfigApiConfigGet = (options) => (options?.client ?? client).get({
    responseType: "json",
    url: "/api/config",
    ...options
  });
  var saveConfigApiConfigPost = (options) => (options.client ?? client).post({
    responseType: "json",
    url: "/api/config",
    ...options,
    headers: {
  ...
  var getConfigDefaultsApiConfigDefaultsGet = (options) => (options?.client ?? client).get({
    responseType: "json",
  ...
  var queueDefaultsEl = requireElement("queue-defaults");
  var queueTroubleshootingEl = requireElement(
  ...
  var copyQueueDefaultsButton = requireElement(
    "copy-queue-defaults"
  );
  ...
  async function refreshQueueStatus({
    ...
    try {
      const response = await getQueueStatusApiQueueStatusGet({
        query: queueName ? { queue_name: queueName } : void 0,
        throwOnError: true
      });
